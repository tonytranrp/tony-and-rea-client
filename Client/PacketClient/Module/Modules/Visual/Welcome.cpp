...
Welcome::Welcome() : IModule(0x0, Category::VISUAL, "Welcomes you at the top of the screen") {
	...
	TextMode.addEntry("Victory++ style", 2);
	TextMode.addEntry("Hello there", 3);
	TextMode.addEntry("Fuck you nigga", 4);
...
void Welcome::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.isInGame() && GameData::canUseMoveKeys()) {
		...
		case 1:
			name = "Welcome's " + playerName + " Daddy";
			break;

		case 2:
			name = "anata wa chiisana oppai wa motsu te iya masu . " + playerName;
			break;

		case 3:
			name = "Hello there sexy! " + playerName;
			break;
		case 4:
			name = "Nigga you sucks " + playerName + " at cpvp";
			break;
		}
		...
		DrawUtils::drawCenteredString(vec2_t(windowSize.x / 2, windowSize.y - windowSize.y + 6.5), &name, scale, MC_Color(interfaceColor), true);
		if (outline)
