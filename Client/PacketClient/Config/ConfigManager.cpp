...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (configExists) {
			...
			currentConfigObj["from"] = "NukeClient";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			g_Data.getGuiData()->displayClientMessageF("[Nuke] %sSuccessfully %s config %s%s%s!", GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
		}
	} else {
		//if (g_Data.getLocalPlayer() != nullptr) 
