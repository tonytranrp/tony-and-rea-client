...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[Nuke] %sCouldn't find player: %s!", RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[Nuke] %s%s is now your friend!", GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[Nuke] %s%s has been removed from your friendlist!", GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[Nuke] %s%s was not in your friendlist!", GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
