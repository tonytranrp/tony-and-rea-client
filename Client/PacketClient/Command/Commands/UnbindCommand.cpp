...
bool UnbindCommand::execute(std::vector<std::string>* args) {
	...
	if (moduleName == "all") {
		if (args->size() >= 3 && args->at(2) == "force") {
			...
			clientMessageF("[Nuke] %sUnbound all modules!", GREEN);
		} else {
			clientMessageF("[Nuke] %sAre you sure? This will unbind %sALL%s%s modules!", RED, BOLD, RESET, RED);
			clientMessageF("[Nuke] %sUse %s.unbind all force%s to unbind all modules", RED, WHITE, RED);
		}
		return true;
	}
	...
	if (!modOpt.has_value()) {
		clientMessageF("[Nuke] %sCould not find module with name: %s", RED, moduleName.c_str());
		return true;
	} 

	auto mod = modOpt.value();
	mod->setKeybind(0x0);
	clientMessageF("[Nuke] %sSuccessfully unbound %s!", GREEN, mod->getRawModuleName());
	return true;
}
